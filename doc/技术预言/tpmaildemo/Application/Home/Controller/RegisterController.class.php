...
class RegisterController extends Controller {
    public function index(){
		...
		$result = $User ->getFieldByUsername($username,'id');;
		//dump($User->_sql());
		$num = count($result);
		//dump($num);
		if($num==1){
		...
		if($User->add($data)){//写入成功，发邮件
         ...
         $mailbody         =     "亲爱的".$data['username']."：<br/>请点击链接激活您的帐号。<br/><a href='http://localhost/demo/tp/index.php/home/register/active.html?verify=".$token."' target='_blank'>http://www.helloweba.com/demo/register/active.php?verify=".$token."</a><br/>如果以上链接无法点击，请将它复制到你的浏览器地址栏中进入访问，该链接24小时内有效。<br/>如果此次激活请求非你本人所发，请忽略本邮件。<br/><p style='text-align:right'></p>";//邮件内容
